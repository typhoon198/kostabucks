      ...
      $(function () {
        ...
        $menuObj.click(function () {
          ...
          switch (href) {
            ...
            case './viewcart':
            case './orderlist':
              $section.load(href, function (responseTxt, statusTxt, xhr) {  //get방식 요청
  ...
  <body>
    ...
    <nav>
      ...
      <c:when test="${empty sessionScope.loginInfo}">
        ...
        <c:if test="${!empty sessionScope.loginInfo}"><!-- 로그인이 된경우 -->
          <li><a href="./orderlist">주문목록</a></li>
        
        </c:if>
        
        <!-- 로그인,가입은 정적페이지 -->
        <!-- 상품목록은 동적페이지(어떤 DB를 쓰는지 어떤페이지를 요청했는지 에 따라 달라지는 페이지 -->
        
      </ol>
    </nav>
    <section>
      <article>
        여름의 시작을 함께할 인도네시아 웨스트 자바. 허브의 복합적인 풍미와 낮은
        산미, 은은하게 달콤한 바닐라향이 매력적인 원두 자세히 보기
      </article>
      <article>
        차별화된 커피 경험을 누릴 수 있는 리저브 매장, 다양한 방법으로 편리하게
        즐길 수 있는 드라이브 스루 매장, 함께해서 더 따뜻할수 있는 지역사회 소통
        공간 커뮤니티 매장
      </article>
    </section>
    ...
    <footer>
      ...
      데이비드 호섭 TEL : 1522-3232 개인정보 책임자 : 하익성 ⓒ 2021 Starbucks
